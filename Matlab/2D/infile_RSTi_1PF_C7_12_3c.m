% This is the input file of three-component and single-phase flow. The up and
% down boundaries are closed and the other boundaries are open. At the initial 
% time, there are C3H8 and C5H12 in the well. Then we impose pressures on the right boundary 
% and inject CH4 into the well from the left boundary. The temperature is constant 
% in the whole process. 

% Author: Yuanqing Wu. Email: wuyuanq@gmail.com
% Last edited on October 24th, 2012
...
% the third component is CO2
model.ct(3) = 304.4; % unit: K
model.cp(3) = 7.4*1.D6; % unit: Pa
model.cv(3) = 0.0021; % unit: m^3/kg
model.af(3) = 0.228;
model.mw(3) = 0.044;
...
model.xInit(2, 1:end, 1:end) = 0.6; 
model.xInit(3, 1:end, 1:end) = 0.4; 

uconst = 2.0*1.D-6; % unit: m/s
model.uBdryX = zeros(2, model.ny); 
model.uBdryY = zeros(model.nx, 2);  
model.uBdryX(1,1) = -uconst; % minus means injection
model.uBdryY(1,1) = -uconst;

% load initial pressures
initPres = zeros(1,ny);
temp = load('../../InitialReservoirData/P9.txt');
k = 1;
for i = 1 : ny
    initPres(i) = temp(k);
    k = k + 1;
...
model.pBdryX(2,1) = initPres(1);
model.pBdryY(end,1) = initPres(1);
