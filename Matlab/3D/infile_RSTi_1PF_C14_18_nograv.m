...
% point is at the front-left-down corner and the production point is at the back-right-up
...
pconst = 20*10^5; % unit: Pa
model.pBdryX = zeros(2, model.ny, model.nz); 
model.pBdryY = zeros(model.nx, 2, model.nz); 
model.pBdryZ = zeros(model.nx, model.ny, 2); 
model.pBdryX(2, end, end) = pconst;
model.pBdryY(end, 2, end) = pconst;
model.pBdryZ(end, end, 2) = pconst;

model.pInit = zeros(model.nx, model.ny, model.nz); 
model.pInit(1:end, 1:end, 1:end) = pconst;

model.soludoc = 'case14';
