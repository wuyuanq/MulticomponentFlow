...
% point is at the front-left-down corner and the production point is at the front-right-down
...
model.pBdryX(2, 1, 1) = pconst;
model.pBdryY(end, 1, 1) = pconst;
model.pBdryZ(end, 1, 1) = pconst;

model.pInit = zeros(model.nx, model.ny, model.nz); 
model.pInit(1:end, 1:end, 1:end) = pconst;

model.soludoc = 'case12';
