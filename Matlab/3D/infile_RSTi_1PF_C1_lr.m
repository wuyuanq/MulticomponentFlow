% This is the input file of two-component and single-phase flow. The up and
% down boundaries are closed and the other boundaries are open. At the initial 
% time, there is C3H8 in the well. Then we impose pressures on the right boundary 
...
model.xBdryX(1, 1, 1:end, 1:end) = 1.0;
model.xBdryX(2, 1, 1:end, 1:end) = 0.0;
model.xBdryX(1, 2, 1:end, 1:end) = 0.0;
model.xBdryX(2, 2, 1:end, 1:end) = 1.0;
model.xBdryY(1, 1:end, 1, 1:end) = 0.0;
model.xBdryY(2, 1:end, 1, 1:end) = 1.0;
model.xBdryY(1, 1:end, 2, 1:end) = 0.0;
model.xBdryY(2, 1:end, 2, 1:end) = 1.0;
model.xBdryZ(1, 1:end, 1:end, 1) = 0.0;
model.xBdryZ(2, 1:end, 1:end, 1) = 1.0;
model.xBdryZ(1, 1:end, 1:end, 2) = 0.0;
...
model.uBdryX(1, 1:end, 1:end) = -2*10^(-6); % unit: m/s
...
for k = 1 : nz
    model.pBdryX(2, 1:end, k) = initPres;
end
