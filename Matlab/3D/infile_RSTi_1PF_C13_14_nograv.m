...
% point is at the front-left-down corner and the production point is at back-right-down
...
model.pBdryX(2, 1, end) = pconst;
model.pBdryY(end, 1, end) = pconst;
model.pBdryZ(end, 1, 2) = pconst;

model.pInit = zeros(model.nx, model.ny, model.nz); 
model.pInit(1:end, 1:end, 1:end) = pconst;

model.soludoc = 'case13';
